...
### P6 — Tích điểm + phân hạng ✅ HOÀN THÀNH
- [x] `V9__loyalty.sql`: point_transactions, tier_history; thêm `points_balance`, `lifetime_points`, `tier` vào users. (V8 đã dùng cho ảnh poster/backdrop phim.) **Gotcha:** CHECK cho `tier` phải khai INLINE cùng cột trong `ALTER TABLE ... ADD` — tách thành `ADD CONSTRAINT ... CHECK (tier...)` riêng thì SQL Server phân giải tên trong cùng batch trước khi cột tồn tại → "Invalid column name 'tier'".
- [x] Quy tắc: **1 điểm / 10.000 ₫** (`floor(total/10000)`), tích trong cùng transaction markPaid (guard `markPaidIfPending` → đúng 1 lần/đơn). Hạng theo **lifetime_points** — hàm thuần `TierPolicy.tierFor`, không job hạ hạng; đổi điểm chỉ trừ `points_balance` nên không tụt hạng. **Ngưỡng thực dùng (demo-friendly): GOLD ≥ 100, PLATINUM ≥ 300** (thấp hơn mô tả gốc 5.000/15.000 để lên hạng tự nhiên khi mua vài vé lúc bảo vệ — đặt trong `TierPolicy`, dễ chỉnh).
- [x] Slice `com.linhvecac.loyalty` (PointTransaction/TierHistory + repo, TierPolicy, LoyaltyService, LoyaltyController). API `GET /api/loyalty/summary` + `GET /api/loyalty/points-history` (`@PreAuthorize hasRole('USER')`); `UserResponse`/`GET /api/users/me` trả thêm `pointsBalance/lifetimePoints/tier`; `BookingResponse.earnedPoints`. FE: tab "Điểm thưởng" (`PointsPage.vue`) + `TierProgressBar.vue` + badge hạng (`BaseBadge` variant `platinum`) ở header tài khoản; "+X điểm" trên trang kết quả thanh toán; `utils/tier.js`.
- **Kiểm tra:** ✅ `mvnw test` xanh (34 test — `LoyaltyServiceTest`: pointsFor floor, biên ngưỡng tierFor 99/100/300, awardForBooking cộng điểm + ghi sổ, đơn <10k không tích, vượt ngưỡng lên GOLD + ghi tier_history; `BookingServiceTest`: markPaid tích điểm 1 lần + gọi lại không tích); Flyway V9 apply + `ddl-auto=validate` xanh; `npm run build` sạch.

### P7 — Voucher, chiến dịch KM, tự động áp dụng, ưu đãi cá nhân hóa ✅ HOÀN THÀNH
- [x] **Mô hình 2 bảng** (gọn hơn 3 bảng gốc): `V10__promotions.sql` (campaigns = định nghĩa quy tắc + user_vouchers = phiếu người dùng) + `V11__seed_promotions.sql` (2 campaign đổi điểm + 4 template OfferEngine). `birth_date` đã có từ V2. Idempotency OfferEngine bằng filtered unique index `ux_user_vouchers_grant(campaign_id,user_id,period_key)`.
- [x] Voucher: PERCENT (cap `max_discount_amount`) / FIXED; ràng buộc `min_order_amount`, `valid_from/to`, `min_tier`, `points_cost` (đổi điểm — trừ `points_balance` không đụng lifetime), `quantity`/`per_user_limit`. Slice `com.linhvecac.promotion` (Campaign/UserVoucher + enum, VoucherService, CampaignService, OfferEngine + Job).
- [x] **Auto-apply** `VoucherService.resolveForQuote` (findBest): lọc voucher AVAILABLE hợp lệ → discount từng cái → chọn max, tie-break `valid_to` sớm hơn; user override (mã cụ thể) / bỏ (sentinel `NONE`). Giữ chỗ bằng guard-update `reserveIfAvailable` (409 nếu bị chiếm); markPaid → USED; job dọn đơn EXPIRED → voucher về AVAILABLE. **Discount chốt DUY NHẤT server-side** (quote + create).
- [x] **OfferEngine rule-based** (cron 6h sáng + `POST /api/admin/offers/run`, idempotent theo `period_key`): sinh nhật (15%, kỳ=năm), thể loại yêu thích 90 ngày (kỳ=tháng), win-back >60 ngày không mua (20%, kỳ=tháng), quà lên hạng (kỳ=tên hạng).
