...
**Lịch sử + dashboard + hoàn thiện (P8):** BE slice `com.linhvecac.dashboard` (KHÔNG có bảng/migration mới — chỉ đọc): `DashboardController` `GET /api/admin/dashboard?days=30` (mặc định 30, clamp ≤365) → `DashboardResponse{summary, revenueDaily, topMovies, tierDistribution}`; bảo vệ tự động qua `SecurityConfig` `/api/admin/**`. `DashboardService` (`@Transactional(readOnly)`): stat hôm nay/hôm qua trả **cả 2 trị thô** (`SummaryStats`), FE tự tính ±% (tránh chia 0 server-side); **fill 0 cho ngày trống** trong cửa sổ N ngày + **đủ 3 hạng** kể cả count 0. Query aggregate thêm vào repo có sẵn (không tạo repo mới): `BookingRepository.{countByStatus, sumRevenueBetween, revenueByDaySince(native), topMoviesByRevenue(Pageable)}`, `BookingSeatRepository.countSoldBetween` (vé CONFIRMED của đơn PAID), `UserRepository.{countNewMembersBetween, countByTier}`. **Gotcha kinh điển P8:** `CAST(b.createdAt AS date)` trong **constructor-expression JPQL** không được Hibernate nhận (validate fail lúc bootstrap) → `revenueByDaySince` phải là **native query** `CONVERT(date, created_at)` trả `List<Object[]>`, map `[java.sql.Date, Number, Number]`→`DailyRevenue` trong service; các aggregate khác (topMovies `new TopMovie(...)`, tierDistribution `new TierCount(u.tier, COUNT)`) vẫn JPQL constructor-expression bình thường vì không có CAST. Doanh thu neo theo `booking.created_at` (không có cột `paidAt`). FE: `pages/admin/DashboardPage.vue` (mở rộng từ bản gọn P3) — 4 stat card ±%, `components/charts/{BarChart(div),DonutChart(SVG stroke-dasharray)}.vue` **tự vẽ, không thêm dependency chart**, bảng top 5 phim, màu hạng donut từ DESIGN §1.2; `api/admin.js` thêm `adminGetDashboard`. **Lịch sử member có filter (client-side)**: `components/ui/FilterChips.vue` (segmented dark-theme) dùng ở 3 tab sẵn có — `MyBookingsPage` (theo trạng thái đơn), `PointsPage` (cộng delta≥0 / trừ delta<0), `VouchersPage` (AVAILABLE+RESERVED / USED / EXPIRED); **không đụng backend** (lọc `computed` trên list đã tải, dữ liệu nhỏ). Test: `DashboardServiceTest` (zero-fill ngày trống, đủ 3 hạng, map stat). `README.md` gốc hướng dẫn chạy.
