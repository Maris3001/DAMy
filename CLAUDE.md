...
Trạng thái: **P5 hoàn thành** — monorepo scaffold + kết nối DB (P1); auth JWT hoàn chỉnh (P2): đăng ký/đăng nhập/hồ sơ, role USER/ADMIN, admin seed `admin@linhvecac.vn` / `Admin@123456` (dev); catalog + admin CRUD (P3); luồng đặt vé 6 bước + giữ ghế chống race condition (P4); thanh toán VNPay/mock + sinh vé (P5). Các phase tiếp theo (P6→P8) theo `PLAN.md`. Cập nhật file này khi cấu trúc thực tế thay đổi.
